...
values ('$numeroRadicado', now()," . $_SESSION['tipo'] . ",3," . $_SESSION['codigo_orfeo'] . ",to_date('" . date('d') . "/" . date('m') . "/" . date('Y') . "','dd/mm/yyyy'),'COLOMBIA'," . $_SESSION['muni'] . "," . $carpeta . "," . $_SESSION['depto'] . ",1,'" . $descripcionAnexos . "', ";
...
values('" . $numeroRadicado . "','" . $numeroRadicado . sprintf("%05d", ($i + 1)) . "'," . $tipoCodigo . "," . $uploader->sizes[$i] . ",'S','" . $rs_login->fields['usua_login'] . "','" . $asuntosNom[$i] . " documento de soporte No. " . $soporNo . "',1,'" . $uploader->nombreOrfeo[$i] . "','N',0,0,0,1,1,'" . $_SESSION["depeRadicaFormularioWeb"] . "',now(),0)";
    ...
    $pdf->SetFont('Arial', '', 8);
    $pdf->Text(100, 51, textoPDF("http://www.uceva.edu.co/index.php/servicios-uceva/"));
    $pdf->SetFont('Arial', '', 8);
    $pdf->Text(100, 54, textoPDF("peticiones-quejas-reclamos-y-felicitaciones/consultaWeb/"));
    $pdf->Text(20, 67, textoPDF("Tulua, " . date('d') . " de " . nombremes(date('m')) . " de " . date('Y')));
    ...
    if ($_SESSION['email'] != "" && $_SESSION['email'] != "No registra") {
        ...
        $mail->Password = "Ucev@2018"; // SMTP account password
        ...
        <body><p>
        Valledupar, " . date('d') . " de " . nombremes(date('m')) . " de " . date('Y') . " <br>
        <br></br>
        ...
        $mensaje .= "<br><br> Para consultar su solicitud <a href='http://192.168.14.10/consultaWeb/consultaWeb/' > aqu&iacute;</a>.<br><br>";
        $mensaje .= "<h5>PBX: +57 (2) 2242202 - FAX: +57 (2) 2259051 -  Línea Nacional Gratuita: 018000 96 46 00<br>Horario de Atención: Lunes a Viernes 8:00am. A 12:00m. y 2:30pm. A 6:30pm.<br> Contacto: info@uceva.edu.co - Código Postal 763022</h5>";
                ...
                <ul>
                    <div id="foli3">
                        <p>
                            ...
                            en su equipo o impr&iacute;malo.</p>
                    </div>
                    <div class="buttons">
                        <input type="button" name="Submit" value="Continuar" onclick="window.open('../bodega/<?= $rutaPdf ?>'); window.close();" />
                        <input type="button" name="Cerrar" value="Cerrar" onclick="window.location = 'http://192.168.14.10/formularioWeb/' / >
     </div>
     </ul>
