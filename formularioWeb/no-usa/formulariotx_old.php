...
if($errorFormulario==0){
	...
	/*echo '<br><pre>';
	print_r($adjuntosSubidos); 
	echo '</pre>';*/
	$uploader->adjuntarArchivos();
	echo '----> '.$uploader->adjuntarArchivos();
...
values ('$numeroRadicado', now(),".$_SESSION['tipo'].",2,".$_SESSION['codigo_orfeo'].",to_date('".date('d')."/".date('m')."/".date('Y')."','dd/mm/yyyy'),'COLOMBIA',".$_SESSION['muni'].",".$carpeta.",".$_SESSION['depto'].",1,'". $descripcionAnexos ."', ";
...
values('".$_SESSION['depeRadicaFormularioWeb']."',now(),1,'$numeroRadicado','Radicacion P.Q.R.S',".$_SESSION['usuaRecibeWeb'].",'22222222',2,'".$_SESSION['documento_destino']."','".$_SESSION['depeRadicaFormularioWeb']."')";
	...
	echo $uploader->moverArchivoCarpetaBodega($numeroRadicado);
	//trae usualogin
...
$pdf->SetFont('Arial','',7);
$pdf->Text(110,48,textoPDF("Consulte su trámite en la página web http://www.unicesar.edu.co/index.php/es/"));
...
if ($_SESSION['email']!="" && $_SESSION['email']!="No registra") {
    ...
    $usMailSelect  = 'orfeo@unicesar.edu.co'; 
    ...
    $mail->Password   = "I1B1bzPdFc"; // SMTP account password
...
Para consultar su solicitud da click <a href=\'http://www.unicesar.edu.co/index.php/es/consultaWeb/\' > aqu&iacute;</a>.<br><br>
<center>
<b>'.$_SESSION["entidad"].'</b><br>
Línea gratuita de Quejas y Reclamos: 018000400380 - PBX: +57 5 5842472 <br>
E-mail: Contacto: pqrs@unicesar.edu.co - Condiciones de Uso y Políticas - Contáctenos
Horario: Horario de atención: Lunes a Viernes de 8:00 am a 12:00 m y de 2:00 pm a 6:00 pm
...
// Envio Correo de notificacion de radicacion a funcionario.
        $subject="Ha recibido PQR's desde la pagina Oficial de ".$_SESSION['razonSocial'];
        $cuerpo="
        <html>
        <head>
