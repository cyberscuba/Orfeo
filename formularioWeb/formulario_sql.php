...
$tipoPqr = 4;
$sql_tipo = "select sgd_tpr_codigo,sgd_tpr_descrip,sgd_tpr_termino from sgd_tpr_tpdcumento where sgd_tpr_web=1 and sgd_tpr_tp" . $tipoPqr . "=1 order by sgd_tpr_descrip";
...
$sql_tipoUsuario = "select * from tipo_usuario_grupo order by nombre_tipo_usuario";
$rs_tipoUsuario = $db->conn->Execute($sql_tipoUsuario);
$tipoUsuariosGrupos = "";
while (!$rs_tipoUsuario->EOF) {
    $tipoUsuariosGrupos .= "<option value='" . $rs_tipoUsuario->fields['ID_TIPO_USUARIO'] ."'>" .$rs_tipoUsuario->fields['NOMBRE_TIPO_USUARIO']  . "</option selected='selected'>";
