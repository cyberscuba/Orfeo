...
<html>
    <head>
        <link rel="stylesheet" href="<?= $url_raiz . $ESTILOS_PATH ?>orfeo.css"><script type="text/javascript" language="javascript">
    ...
    <body><form action=correspondencia.php method="get">
            ...
            if ($usualoginuevo == trim($krd)) {
                ...
                if (trim($contraxx)) {
                    ...
                    $isqlConsulta = "select CARP_CODI,CARP_DESC from carpeta order by carp_codi ";
...
//                                                    $db->conn->debug = true;
                                                    ...
                                                    if ($numdata == 11) {   // Se realiza la cuenta de radicados en Visto Bueno VoBo
                                                        if ($codusuario == 1) {
                                                            ...
                                                            $isqlCountRad = "select count(*) as CONTADOR from radicado where carp_per=0 and carp_codi=$numdata and radi_depe_actu='$dependencia' and radi_usu_ante='$krd' and radi_nume_deri='0'";
                                                        }
                                                        $addadm = "&adm=1";
                                                    } else {
                                                        $isqlCountRad = "select count(*) as CONTADOR from radicado where carp_per=0 and carp_codi=$numdata and  radi_depe_actu='$dependencia' and radi_usua_actu=$codusuario and radi_nume_deri='0' ";
...
//                                                    error_log('### '.$carpeta. ' ### '.$numdata.' ###');
                                                    
                                                    $flag = 0;
                                                    $rs1CountRad = $db->query($isqlCountRad);
//                                                    error_log('@@@@@@@@@@@@@@@ '.$isqlCountRad);
