...
//                alert("Debe seleccionar por lo menos un tipo documental datosAguardar " +document.getElementById("datosAguardar").value);
                var numerico = 0;
...
//    echo '</pre>'; 
    ...
    for ($i = 0; $i < count($tipodocumental); $i++) {
        $isql1Rolss = "select cod_tp_tpdcumento as COD_TP_TPDCUMENTO from rol_tipos_doc where cod_rol=" . $codigoRol . " and cod_tp_tpdcumento=" . $tipodocumental[$i];
        ...
        if ($rsRolss->fields["COD_TP_TPDCUMENTO"] == '') {
            ...
            if ($driver == 'postgres') {
                ...
                $insertrol = "insert into rol_tipos_doc values ($conteoTotaol,$codigoRol,$tipodocumental[$i],1)";
            } else {
                $insertrol = "insert into rol_tipos_doc values ($codigoRol,$tipodocumental[$i],1)";
            }
