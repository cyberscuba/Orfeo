            ...
            <table border=1 width=100% class=t_bordeGris align="center">
                ...
                if (isset($_POST['reg_crear'])) {
                    ...
                    if ($rsrol->fields["NOMB_ROL"]) {
                        echo "<tr><td align='center' colspan='2'><center><p><span class=etexto color='red'><B>El rol ingresado ya existe</B></span></p></center></td></tr>";
                    } else {
                        $esta = $_POST['estadorol'];
                        $isertrol = "insert into roles (nomb_rol, fecha, estado) values('" . strtoupper($_POST['rol']) . "','" . date('Y-m-d') . "','$esta')";
                        $rs = $db->query($isertrol);
                        if ($rs) {
                            echo "<tr><td align='center' colspan='6'><center><p><span class=etexto color='red'><B>Se cre&oacute; el rol de forma correcta</B></span></p></center></td></tr>";
                            echo "<script>";
                            echo "recargar();";
                            echo "</script>";
                        }
...
//                        echo '******************** '.$selectRoles;
                        if ($rsselectRoles->fields["COUNT"] > 0) {
                            ...
                            $rs = $db->query($updaterol);
                            if ($rs) {
                                $mensaje = 'Se actualiz&oacute; el rol de forma correcta';
                            }
                        }
                    } else {
                        $updaterol = "update roles set nomb_rol='" . strtoupper($_POST['rol']) . "', estado= '$esta' where nomb_rol='" . $_POST['codrol'] . "'";
