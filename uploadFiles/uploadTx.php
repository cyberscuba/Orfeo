...
//if (!$dependencia or !$nivelus)  include "./rec_session.php";
    ...
    <body>
        ...
        $sql = 'SELECT RADI_DEPE_ACTU as "DEPENDENCIA", RADI_NUME_RADI as "RADICADO", RADI_USUA_ACTU AS "USUARIO" FROM RADICADO WHERE RADI_NUME_RADI=\'' . $valRadio . "'";
//        $db->conn->SetFetchMode(ADODB_FETCH_ASSOC);
        $rs = $db->conn->query($sql);
//        echo '*************** '.$sql;
        $radi = $rs->fields['radicado'];
        $depe = $rs->fields['dependencia'];
        $usua = $rs->fields['usuario'];

        $tipoRad = substr($radi, -1);

        if ($tipoRad == 1) {
            echo "<script>window.open(\"mail.php?dependencia=$depe&usunom=$usua&verrad=$radi&radi_nume=$radi&asunto=$observa&tx=Asociar&codusu=$usua\", 'Modificacion_de_Datos', 'height=300,width=350,scrollbars=yes');</script>";
        }
        ?>
    <center>
        ...
        <table class="borde_tab">
            <tr><td class="titulosError">
                    ...
                    $query = "update radicado  set radi_path='" . strtoupper(substr($fileGrb, 0, -3)) . "pdf'
