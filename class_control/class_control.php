...
//        error_log('*** ' . $tipoanexo . ',' . $cuentai . ',' . $documento_us3 . ',' . $med . ',' . $fec . ',' . $radicadopadre . ',' . $codusuario . ',' . $tip_doc . ',' . $ane . ',' . $pais . ',' . $asu . ',' . $coddepe . ',' . $tip_rem . ',' . $numdoc . ',' . $tdoc . ',' . $muni_codi . ',' . $archivo . ',' . $usua_doc . ',' . $depe_codi_territorial . ',' . $secRadicacion . ',' . $numeroExpediente . ',' . $driver);
        if (count($tmp) == 3) {
            $muni_codi = $tmp[1];
            $dpto_codi = $tmp[2];
            $pais_codi = '170';
