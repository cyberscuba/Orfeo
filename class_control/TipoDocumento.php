...
class TipoDocumento {
    ...
    function TipoDocumento_codigo($codigo) {
        if (strlen($codigo) > 0) {
            ...
            if (!$rs->EOF) {
                ...
                $this->sgd_tpr_descrip = $assoc == 0 ? $rs->fields['sgd_tpr_descip'] : $rs->fields['SGD_TPR_DESCIP'];
    ...
    function getNombreTipodoc($codigo) {
        $qsss = "select sgd_tpr_descip as SGD_TPR_DESCIP from sgd_tpr_tpdcumento where sgd_tpr_codigo=$codigo";
        $rssss = $this->cursor->query($qsss);
        $nombre = $rssss->fields['SGD_TPR_DESCIP'];
        return $nombre;
    }
