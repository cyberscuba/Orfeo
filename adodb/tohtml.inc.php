...
//        error_log('------- '.$prefijo);
            ...
            ***/
            ...
            if (substr($fname, 0, 4) == "CHU_") {
                ...
                ***/
                ...
                Autor: Andrés Mosquera
                Fecha: 17-10-2018
                Información: Se comenta esta variable ya que se hace mas organizada arriba
                ***/
                ...
                Autor: Andrés Mosquera
                Fecha: 17-10-2018
                Información: Fin Se comenta esta variable ya que se hace mas organizada arriba
                ***/
    ...
    while (!$rsTmp->EOF) {
        ...
        /* by Skinatech */
        /* Esta parte es la que controla los permisos de acceso a los radicados de los anexos y los tipos documentales */
        $consultaTotalTipo = "select sgd_tpr_codigo as SGD_TPR_CODIGO from sgd_tpr_tpdcumento where sgd_tpr_descrip='" . $rsTmp->fields["TIPO DOCUMENTO"]."'";
                    ...
                    if ($pathImagen) {
                        ...
                        if ((($_SESSION['codusuario'] != $usuario_actual) && ($_SESSION['depecodi'] != $dependencia_actual) && $permission == 1) || ($nivel_radicado > $nivel_usuario)
                            ...
                            if (in_array($tdocper, $tiposdocumentales)) {
                                ...
                                $v = "<a href='#' onclick=\"alert('Usted no tiene acceso al tipo documental ".$rsTmp->fields["TIPO DOCUMENTO"].". Comun&iacute;quese con el administrador')\"><span class=$radFileClass'>$v</span></a>";
                            }

                            /* $rutaRaiz = $_SESSION['url_raiz'];
...
//                    $v = "<a href=" . $url_raiz . "/verradicado.php?verrad=" . $verNumRadicado . "&" . $encabezado . " title='Ver informacion del radicado' alt='Ver informacion del radicado' aria-label='Abrir Información del radicado' ><span class=$radFileClass>" . $v . "</span></a>";
                ...
                case 'HID_';
                        if($rsTmp->fields['HID_PATH_ACTA'] != ''){
                            ...
                            $v = "<a href='".$rutaRaiz."/bodega$pathActa' title='Ver informacion del radicado' alt='Ver informacion del radicado' aria-label='Abrir Información del radicado' ><span class=$radFileClass>".$v."</span></a>";
                        }
			break;
            }
            $type = $typearr[$i];
