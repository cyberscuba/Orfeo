...
//            $this->db->conn->query($query);
            if (!$this->db->conn->query($query)) {
                error_log('<br>Lo siento no pudo agregar el expediente<br> '.$query);
    ...
    function crearExpediente($numExpediente, $radicado, $depe_codi, $usua_codi, $usua_doc, $usuaDocExp, $codiSRD, $codiSBRD, $expOld = null, $fechaExp = null, $codiPROC = null, $arrParametro = null) {
        ...
        if ($expOld == "false") {
            ...
            if ($expManual == 1) {
                ...
                $secExp = substr($numExpediente, 11, 6);
            }
...
//    }
    ...
    function expedientesRadicado($radicado) {
        $query = "SELECT SGD_EXP_NUMERO FROM SGD_EXP_EXPEDIENTE";
        $query .= " WHERE RADI_NUME_RADI = '" . $radicado . "' AND SGD_EXP_ESTADO <> 2";
        $rs = $this->db->conn->query($query);
        error_log('********************** Expediente.php '.$query);
        if ($rs->EOF) {
            $arrExpedientes[0] = 0;
        } else {
            $e = 0;
