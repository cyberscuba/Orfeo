    ...
    case 'mssql': {
            if ($tipoDocumento == '9999') {
                $queryE = "SELECT b.USUA_NOMB as USUARIO, 
					...
					FROM RADICADO r 
						INNER JOIN USUARIO b ON r.radi_usua_radi=b.usua_CODI AND $tmp_substr($radi_nume_radi,$num,$longitud_codigo_dependencia)=cast(b.depe_codi as varchar($longitud_codigo_dependencia))
    ...
    case 'mysql': {
            if ($tipoDocumento == '9999') {
                $queryE = "SELECT b.USUA_NOMB as USUARIO, 
                        ...
                        MIN(depe_codi) as HID_DEPE_USUA 
                    FROM RADICADO r 
                            INNER JOIN USUARIO b ON r.radi_usua_radi=b.usua_CODI AND $tmp_substr($radi_nume_radi,$num,$longitud_codigo_dependencia)=cast(b.depe_codi as char($longitud_codigo_dependencia))
                    WHERE " . $db->conn->SQLDate('Y/m/d', 'r.radi_fech_radi') . " BETWEEN '$fecha_ini' AND '$fecha_fin' 
                            $whereDependencia $whereActivos $whereTipoRadicado 
                    GROUP BY b.USUA_NOMB ORDER BY $orno $ascdesc";
            } else {
...
function pintarEstadistica($fila, $indice, $numColumna) {
    global $dir_raiz, $_POST, $_GET, $krd;
    $salida = "";
...
function pintarEstadisticaDetalle($fila, $indice, $numColumna) {
    global $dir_raiz, $encabezado, $krd;
    ...
    switch ($numColumna) {
        ...
        case 1:
            if ($radipath && $verImg) {
                ...
                $salida = "<center><a href=\"{$dir_raiz}bodega" . $rutaBodega . "\">" . $radicado . "</a></center>";
            } else {
        ...
        case 2:
            if ($verImg)
                $salida = "<a class=\"vinculos\" href=\"{$dir_raiz}verradicado.php?verrad=" . $radicado . "&amp;" . session_name() . "=" . session_id() . "&amp;krd=" . $_GET['krd'] . "&amp;carpeta=8&amp;nomcarpeta=Busquedas&amp;tipo_carp=0 \" >" . $fecharadi . "</a>";
            else
