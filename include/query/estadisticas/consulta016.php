...
function pintarEstadistica($fila, $indice, $numColumna) {
    ...
    if (isset($fila['FECH_RADI'])) {
        $fechRadi = $fila['FECH_RADI'];
        $radicado = $fila['RADICADOS'];
        $hidFEch = $fila['HID_FECH_SELEC'];
        $totalRespuesta = $fila['RAD_SALIDAs'];
    } else {
        $fechRadi = $fila[0];
        $radicado = $fila[1];
//        $totalRespuesta = $fila[4];
        $totalRespuesta = $fila[2];
    }
...
//    $numRadicado = $fila['RAD_ENTRADA'];
    switch ($numColumna) {
        ...
        case 2:
            if ($verImg)
                $salida = "<a class=\"vinculos\" href=\"../verradicado.php?verrad=" . $radEntrada . "&amp;" . session_name() . "=" . session_id() . "&amp;krd=" . $_GET['krd'] . "&amp;carpeta=8&amp;nomcarpeta=Busquedas&amp;tipo_carp=0 \" >" . $fechRadiE. "</a>";
            else
