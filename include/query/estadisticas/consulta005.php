    ...
    case 'postgres': {
            ...
            $queryE = "
				...
				WHERE 
					...
					AND h.SGD_TTR_CODIGO=2
					AND " . $db->conn->SQLDate('Y/m/d', 'r.radi_fech_radi') . " BETWEEN '$fecha_ini' AND '$fecha_fin' 
					AND cast(r.RADI_NUME_RADI as varchar(30)) like '%2'
					and r.mrec_codi=m.mrec_codi
				$whereTipoRadicado 
				GROUP BY b.USUA_NOMB
				ORDER BY $orno $ascdesc";
...
function pintarEstadistica($fila, $indice, $numColumna) {
    ...
    switch ($numColumna) {
        ...
        case 1:
            ...
            break;
        case 2:
            $datosEnvioDetalle = "tipoEstadistica=" . $_POST['tipoEstadistica'] . "&amp;genDetalle=1&amp;usua_doc=" . urlencode($fila['HID_USUA_DOC']) . "&amp;dependencia_busq=" . $_POST['dependencia_busq'] . "&amp;fecha_ini=" . $_POST['fecha_ini'] . "&amp;fecha_fin=" . $_POST['fecha_fin'] . "&amp;tipoRadicado=" . $_POST['tipoRadicado'] . "&amp;tipoDocumento=" . $_POST['tipoDocumento'] . "&amp;codUs=" . $hidcodusu . "&amp;depeUs=" . $hiddepeusua;
    ...
    switch ($numColumna) {
        ...
        case 1:
            if ($hidradipat && $verImg)
                $salida = "<center><a href=\"{$ruta_raiz}bodega" . $fila['RADI_PATH'] . "\">" . $radicado . "</a></center>";
            else
        ...
        case 4:
            if ($verImg)
                $salida = "<a class=\"vinculos\" href=\"{$ruta_raiz}verradicado.php?verrad=" . $radicado . "&amp;" . session_name() . "=" . session_id() . "&amp;krd=" . $_GET['krd'] . "&amp;carpeta=8&amp;nomcarpeta=Busquedas&amp;tipo_carp=0 \" >" . $fecharadica . "</a>";
            else
